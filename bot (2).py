...
async def process_config_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if on_cooldown:
        await message.reply_text(
            f"⏱️ <b>Please wait {remaining}s before next request!</b>",
            parse_mode='HTML'
        )
        return
...
📁 <b>Supported:</b> .ehi, .dark, .txt, .ovpn, .conf",
            parse_mode='HTML'
        )
        return
...
def run_webhook():
    ...
    # Start Flask server (this binds to PORT)
